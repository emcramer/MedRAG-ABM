...
class MedRAG:
    ...
    def __init__(self, llm_name="OpenAI/gpt-3.5-turbo-16k", rag=True, retriever_name="MedCPT", corpus_name="Textbooks", db_dir="./corpus", cache_dir=None):
        ...
        self.templates = {"cot_system": general_cot_system, "cot_prompt": general_cot,
                    "medrag_system_mc": general_medrag_system_mc, "medrag_prompt_mc": general_medrag_mc,
                    "medrag_system_yn": general_medrag_system_yn, "medrag_prompt_yn": general_medrag_yn,
                    "abm_medrag_temp":abm_medrag_temp, "abm_medrag_prompt":abm_medrag_prompt}
    ...
    def answer(self, question, options=None, k=32, rrf_k=100, save_dir=None, mc=False, abm=False, cell_type=None, signal=None, snippets=None, snippet_ids=None):
        '''
        ...
        if not self.rag:
            ...
            if mc: 
                prompt_ = "medrag_prompt_mc"
                sys_ = "medrag_system_mc"
            elif abm:
                prompt_ = "abm_medrag_temp"
                sys_ = "abm_medrag_prompt"
            else:
                prompt_ = "medrag_prompt_yn"
                sys_ = "medrag_system_yn"
            if abm:
                prompt_medrag = self.templates[prompt_].render(context=context, cell_type=cell_type, signal=signal)
            else:
                prompt_medrag = self.templates[prompt_].render(context=context, question=question, options=options)
            messages=[
                    {"role": "system", "content": self.templates[sys_]},
